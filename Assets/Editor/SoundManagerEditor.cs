...
public class SoundManager : MonoBehaviour
{
    ...
    // =========================
#if UNITY_EDITOR
    [ContextMenu("Auto Load Sounds")]
    public void AutoLoadSounds()
    {
        // 사전 초기화 (null 방지)
        if (soundDictionary == null) soundDictionary = new Dictionary<SoundType, AudioClip[]>();
        if (loadedSoundCounts == null) loadedSoundCounts = new Dictionary<SoundType, int>();

        soundDictionary.Clear();
        loadedSoundCounts.Clear();
        ...
        foreach (SoundType soundType in Enum.GetValues(typeof(SoundType)))
        {
            ...
            if (clips.Length > 0)
            {
                ...
                soundDictionary[soundType] = clips;
                loadedSoundCounts[soundType] = clips.Length;
                Debug.Log($"[AutoLoad] {soundType} : {clips.Length} clips");
            }
            else
            {
                Debug.LogWarning($"[AutoLoad] No clips for {soundType} in Resources/{folderPath}");
            }
        }
    ...
    [ContextMenu("Scan Sound Folders")]
    public void ScanSoundFolders()
    {
        // Assets/Resources/Sound 하위 폴더 스캔
        string basePath = "Assets/Resources/Sound";
        if (System.IO.Directory.Exists(basePath))
        {
            string[] directories = System.IO.Directory.GetDirectories(basePath);
            Debug.Log("=== Found Sound Folders ===");
            foreach (string dir in directories)
            {
                string folderName = System.IO.Path.GetFileName(dir);
                Debug.Log($"Folder: {folderName}");
            }
            Debug.Log("=== Add these to SoundType enum if missing ===");
        }
        else
        {
